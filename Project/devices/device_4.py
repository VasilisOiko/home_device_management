...
def publish_power_consumption(client):
    ...
    while(1):
        if powerState == "ON":
            ...
            client.publish(PUBLISH_TOPIC, measurment)
...
def on_message(client, userdata, message):
    ...
    elif payload == 'OFF' and publishProcedure.is_alive():
...
if __name__ == '__main__':
    ...
    global publishProcedure
    global powerState
    
    # Create MQTT client instance
    client = mqtt.Client()

    publishProcedure = Thread(target = publish_power_consumption, args = (client, ))
    powerState = "ON"
    
    publishProcedure.start()
